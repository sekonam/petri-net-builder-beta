...
class Transition extends React.Component {
  ...
  render() {
    ...
    return (
      <g className="state" id={data.id}>
        <rect className="state-rect" x={x} y={y}
          width={width + 'px'} height={height + 'px'}
          rx={r} ry={r}  style={data.color ? {fill: data.color} : {}}>
        </rect>
        <text className="state-txt" x={x+7} y={y+25}>{this.props.data.short('name', Math.round(data.width/10))}</text>
        {socketTags}
        <CircleButton clickHandler={(e) => { methods.transition.remove(data.id); e.stopPropagation(); }}
